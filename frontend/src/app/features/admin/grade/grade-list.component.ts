...
interface Grade { id: number; name: string; level: number; description: string; active: boolean; }
...
@Component({
  ...
  template: `
    ...
    <app-data-table [columns]="columns" [rows]="rows()" [page]="currentPage()" [totalPages]="totalPages()" [totalElements]="totalElements()" [loading]="loading()" emptyTitle="No grades defined" (pageChange)="loadPage($event)" (rowClick)="openModal($event)" />

    @if (showModal()) {
      <div class="modal-overlay" (click)="closeModal()">
        <div class="modal-panel animate-fade-in-up" (click)="$event.stopPropagation()">
          <div class="modal-header"><h2>{{ editing() ? 'Edit' : 'Create' }} Grade</h2><button class="btn-close" (click)="closeModal()"><svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M13.5 4.5L4.5 13.5M4.5 4.5l9 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button></div>
...
export class GradeListComponent implements OnInit {
  columns: TableColumn[] = [
    ...
    { key: 'level', label: 'Level', sortable: true, width: '100px', align: 'center' },
  ...
  openModal(item?: Grade): void {
    if (item) { this.editing.set(item); this.form = { name: item.name, level: item.level, description: item.description }; }
    else { this.editing.set(null); this.form = { name: '', level: 1, description: '' }; }
