...
interface DocType { id: number; name: string; description: string; active: boolean; }
...
@Component({
  ...
  template: `
    <app-page-header title="Document Types" subtitle="Manage admission document types">
      <button class="btn btn-gold" (click)="openModal()">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3v10M3 8h10" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
        Add Document Type
      </button>
    </app-page-header>
    <app-data-table [columns]="columns" [rows]="rows()" [page]="currentPage()" [totalPages]="totalPages()" [totalElements]="totalElements()" [loading]="loading()" emptyTitle="No document types" emptySubtitle="Add document types for admission." (pageChange)="loadPage($event)" (rowClick)="openModal($event)" />

    @if (showModal()) {
      <div class="modal-overlay" (click)="closeModal()">
        <div class="modal-panel animate-fade-in-up" (click)="$event.stopPropagation()">
          <div class="modal-header"><h2>{{ editing() ? 'Edit' : 'Create' }} Document Type</h2><button class="btn-close" (click)="closeModal()"><svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M13.5 4.5L4.5 13.5M4.5 4.5l9 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button></div>
          <form class="modal-body" (ngSubmit)="save()">
            <div class="form-group"><label class="form-label">Name <span class="req">*</span></label><input type="text" class="form-control" [(ngModel)]="form.name" name="name" required placeholder="e.g. Transcript"></div>
            <div class="form-group"><label class="form-label">Description</label><textarea class="form-control" [(ngModel)]="form.description" name="description" rows="3" placeholder="Description..."></textarea></div>
...
export class DocumentTypeListComponent implements OnInit {
  ...
  editing = signal<DocType | null>(null);
  saving = signal(false);
  confirmDelete = signal<DocType | null>(null);
  ...
  loadPage(page: number): void {
    ...
    this.crud.list<DocType>('document-types', page, 10).subscribe({
  ...
  save(): void {
    ...
    const obs = this.editing() ? this.crud.update('document-types', this.editing()!.id, this.form) : this.crud.create('document-types', this.form);
    obs.subscribe({
